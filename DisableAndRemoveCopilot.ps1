...
$Property2 = (Get-ItemPropertyValue -Path $RegistryPath2 -Name $Name2)
	if ($Property2 -ne 1) {
		...
		Write-Host 'Checking for Copilot AppxPacakges'
		$CopilotLives = (Get-AppxPackage -AllUsers | Where-Object {$_.Name -like "*Copilot*"})
		If ($Null -ne $CopilotLives) {
		Write-Host 'Uninstalling Copilot AppxPackages'
		...
		Write-Host 'Copilot Uninstalled'	
	}
	}
	else {
		Write-Host 'Copilot Already Disabled, Now Unisntalling Copilot App'	
		Write-Host 'Checking for Copilot AppxPacakges'
		$CopilotLives = (Get-AppxPackage -AllUsers | Where-Object {$_.Name -like "*Copilot*"})
		If ($Null -ne $CopilotLives) {
		Write-Host 'Uninstalling Copilot AppxPackages'
